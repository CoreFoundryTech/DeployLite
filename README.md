...
- Production deployments require `DEPLOYLITE_REPO_ALLOWED_HOSTS`, a comma-separated allowlist of approved HTTPS Git hosts (the installer/example defaults to `github.com`). The agent rejects credentials, non-443 ports, unapproved names, Compose service names, and origins resolving to loopback, private, link-local, carrier-grade, multicast, reserved, IPv6-local, or unique-local addresses. It resolves immediately before Git starts to narrow DNS-rebinding exposure; without an egress proxy, DNS resolution and Git connection still have a residual TOCTOU window.
