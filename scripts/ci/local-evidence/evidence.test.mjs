...
import { ADVISORY_ASSERTIONS, aggregateOutcome, assertBinding, assertCataloguedCheck, canonicalJson, createEvidence, redactExcerpt } from "./evidence.mjs";
...
test("RED contract rejects malformed reason codes instead of serializing ambiguous evidence", () => {
  assert.throws(() => createEvidence({ binding, checks: [{ ...check, reasonCode: 42 }] }));
});
...
test("canonical serialization and hash are deterministic regardless of input key order", () => {
  ...
  const first = createEvidence({ binding, checks: [check] });
  const second = createEvidence({ binding: { ...binding }, checks: [{ ...check, argv: [...check.argv] }] });
...
test("evidence redacts known values and paths and carries non-equivalence assertions", () => {
  ...
  const evidence = createEvidence({ binding, checks: [check], knownValues: ["top-secret"] });
