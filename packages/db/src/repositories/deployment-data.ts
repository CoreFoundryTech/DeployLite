...
export class DbDeploymentRepository implements DeploymentRepository {
  ...
  async rollbackProjection(previous: Deployment, projected: Deployment, eventId: string): Promise<boolean> {
    return this.db.transaction(async (tx) => {
      ...
      const [restored] = await tx.update(deployments).set({ status: previous.status, finishedAt: previous.finishedAt ? new Date(previous.finishedAt) : null, updatedAt: new Date() })
        .where(and(eq(deployments.id, projected.id), eq(deployments.status, projected.status))).returning({ id: deployments.id });
  ...
  async appendLog(event: LogEvent): Promise<LogEvent> {
    return this.db.transaction(async (tx) => {
      ...
      const [row] = await tx.insert(deploymentLogs).values({ ...event, message: redactLogMessage(event.message), redactionApplied: true }).returning();
  ...
  async appendAllocatedLog(event: Omit<LogEvent, "sequence">): Promise<LogEvent> {
    ...
    return this.db.transaction((tx) => appendAllocatedLog(tx, event));
  }
...
async function appendAllocatedLog(tx: Parameters<DeployLiteDb["transaction"]>[0] extends (tx: infer Transaction) => unknown ? Transaction : never, event: Omit<LogEvent, "sequence">): Promise<LogEvent> {
  ...
  const [row] = await tx.insert(deploymentLogs).values({ ...event, sequence: allocation.sequence, message: redactLogMessage(event.message), redactionApplied: true }).returning();
  if (!row) throw new Error("Failed to append deployment log");
  return toLogEvent(row);
}
